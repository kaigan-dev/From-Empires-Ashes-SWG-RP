...
void ChatManagerImplementation::handleChatRoomMessage(CreatureObject* sender, const UnicodeString& message, unsigned int roomID, unsigned int counter) {
	...
	if(auctionRoom != nullptr && auctionRoom->getRoomID() == roomID) {
		...
		//if(channel->getName() == "ooc") {
		//	channel->broadcastMessage(new ChatRoomMessage("Discord", server->getGalaxyName(), formattedMessage, roomID));
		//}
	}
...
void ChatManagerImplementation::broadcastChatMessage(CreatureObject* sourceCreature, const UnicodeString& message,
	...
	String messagePrefix = sourceCreature->getStoredString("chat_prefix");
	if(messagePrefix != "") {
		//message = UnicodeString(messagePrefix + " ") + message;
		//message = UnicodeString(messagePrefix + " " + message.toString());
		//newMessage = messagePrefix + " " + message;
	}
	...
	if (sourceCreature->isPlayerCreature()) {
		...
		if (myGhost != nullptr) {
			...
			if (sourceCreature->getObserverCount(ObserverEventType::SPATIALCHATSENT)) {
				...
				if(messagePrefix != "") 
					chatMessage->setString(messagePrefix + " " + message.toString());
				else 
	...
	if (closeObjects != nullptr) {
		...
		//error(sourceCreature->getFirstName() + " had close objects.");
	} else {
#ifdef COV_DEBUG
		sourceCreature->info("Null closeobjects vector in ChatManager::broadcastChatMessage", true);
#endif
		//Default 128. Let's try 512.
		zone->getInRangeObjects(sourceCreature->getWorldPositionX(), sourceCreature->getWorldPositionY(), 512, &closeEntryObjects, true);
		//error(sourceCreature->getFirstName() + " did not have close objects.");
		//error(sourceCreature->getFirstName() + ": World: X" + String::valueOf(sourceCreature->getWorldPositionX()) + " Y: "+ String::valueOf(sourceCreature->getWorldPositionY()));
		//error(sourceCreature->getFirstName() + ": Local: X" + String::valueOf(sourceCreature->getPositionX()) + " Y: "+ String::valueOf(sourceCreature->getPositionY()));
	}
	...
	try {
		for (int i = 0; i < closeEntryObjects.size(); ++i) {
			...
			if (param == nullptr) {
				if(messagePrefix != "" && spatialChatTypeNames.get(spatialChatType) != "emote")
					cmsg = new SpatialChat(sourceID, targetID, chatTargetID, UnicodeString(messagePrefix + " ") + message, range, spatialChatType, moodType, chatFlags, languageID);
				else 
