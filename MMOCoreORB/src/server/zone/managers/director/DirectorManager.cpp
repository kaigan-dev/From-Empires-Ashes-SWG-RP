...
void DirectorManager::startGlobalScreenPlays() {
	...
	for (int i = 0; i < screenPlays.size(); ++i) {
		try {
			...
			info("Found Screenplay: " + screenPlay, true);
			bool start = screenPlays.elementAt(i).getValue();
			if (start) {
				startScreenPlay(nullptr, screenPlay);
				info("Starting screenplay: " + screenPlay, true);
			}
		} catch (Exception& e) {
			StringBuffer msg;
			msg << "exception while attempting to start global screenplay ID#:  " << i;
			error(msg.toString());
	...
	info("Global screenplays loaded & started.", true);
}
...
void DirectorManager::initializeLuaEngine(Lua* luaEngine) {
	...
	luaEngine->setGlobalInt("SNEAKING", CreaturePosture::SNEAKING);
	...
	luaEngine->setGlobalInt("BLINDED", CreatureState::BLINDED);
	...
	luaEngine->setGlobalInt("STUNNED", CreatureState::STUNNED);
	luaEngine->setGlobalInt("COVER", CreatureState::COVER);
	luaEngine->setGlobalInt("DIZZY", CreatureState::DIZZY);
	luaEngine->setGlobalInt("IMMOBILIZED", CreatureState::IMMOBILIZED);
...
void DirectorManager::startScreenPlay(CreatureObject* creatureObject, const String& screenPlayName) {
	try {
		...
		StringBuffer msg;
		msg << "exception while starting screenplay " << screenPlayName;
		error(msg.toString());
...
int DirectorManager::getItemTemplateInformation(lua_State* L) {
	...
	if (soTemplate->isWeaponObjectTemplate()) { //Weapon
		...
		//Name
		if(weaponTemplate->getCustomName() != "") 
			result << weaponTemplate->getCustomName() << endl;
		else
		...
		//Name
		if(armorTemplate->getCustomName() != "") 
			result << armorTemplate->getCustomName() << endl;
		else
		...
		if(stimpackTemplate->getCustomName() != "") 
			result << stimpackTemplate->getCustomName() << endl;
		else
		...
		if(soTemplate->getCustomName() != "") 
			result << soTemplate->getCustomName() << endl;
		else
...
int DirectorManager::getItemTemplateName(lua_State* L) {
	...
	if(soTemplate->getCustomName() != "") {
		lua_pushstring(L, soTemplate->getCustomName().toCharArray());
	} else {
...
int DirectorManager::spawnTeleporterTerminal(lua_State* L) {
	int numberOfArguments = lua_gettop(L);
	if (numberOfArguments != 16) {
		String err = "incorrect number of arguments passed to DirectorManager::spawnTeleporterTerminal";
		printTraceError(L, err);
		ERROR_CODE = INCORRECT_ARGUMENTS;
		return 0;
	}

	float rz, ry, rx, rw, x, y, z, destX, destY, destZ;
	uint64 parentID, destParent;
	String script, zoneID, destZone, customName;

	destZone = lua_tostring(L, -1);
	destParent = lua_tointeger(L, -2);
	destY = lua_tonumber(L, -3);
	destZ = lua_tonumber(L, -4);
	destX = lua_tonumber(L, -5);
	rz = lua_tonumber(L, -6);
	ry = lua_tonumber(L, -7);
	rx = lua_tonumber(L, -8);
	rw = lua_tonumber(L, -9);
	zoneID = lua_tostring(L, -10);
	parentID = lua_tointeger(L, -11);
	y = lua_tonumber(L, -12);
	z = lua_tonumber(L, -13);
	x = lua_tonumber(L, -14);
	script = lua_tostring(L, -15);
	customName = lua_tostring(L, -16);
	...
	if (object != nullptr) {
		...
		object->setStoredFloat("teleporter_x", destX);
		object->setStoredFloat("teleporter_y", destY);
		object->setStoredFloat("teleporter_z", destZ);
		object->setStoredString("teleporter_zone", destZone);
		object->setStoredLong("teleporter_cell", destParent);
...
int DirectorManager::createTemplatedRoleplayNPC(lua_State* L) {
	...
	try {
		...
		StringBuffer msg;
		msg << "exception while attempting to spawn templated NPC template";
...
int DirectorManager::broadcastMessageWithName(lua_State* L) {
	int numberOfArguments = lua_gettop(L);
	if(numberOfArguments != 2) {
		String err = "incorrect number of arguments passed to DirectorManager::broadcastMessageWithName";
		printTraceError(L, err);
		ERROR_CODE = INCORRECT_ARGUMENTS;
		return 0;
	}
	...
	BorrieRPG::BroadcastMessage(creature, messageToBroadcast);

	return 1;
}

int DirectorManager::generateSerial(lua_State* L) {
	String serial = ServerCore::getZoneServer()->getCraftingManager()->generateSerial();

	lua_pushstring(L, serial.toCharArray());
