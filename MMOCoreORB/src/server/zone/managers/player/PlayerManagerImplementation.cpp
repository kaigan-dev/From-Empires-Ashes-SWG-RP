...
void PlayerManagerImplementation::loadLuaConfig() {
	...
	baseImmersiveExp = lua->getGlobalInt("baseImmersiveExp");
...
bool PlayerManagerImplementation::acceptTeachingOffer(CreatureObject* teacher, CreatureObject* student, Skill* skill) {
	...
	if (teacher->hasSkill(skill->getSkillName()) && skillManager->awardSkill(skill->getSkillName(), student, true, false, false, 1)) {
		...
		//Give EXP for training force skills.
		if(BorSkill::GetSkillIsForceSkill(skill->getSkillName())) {
			StringIdChatParameter params("teaching", "experience_received"); //You have received %DI Apprenticeship experience.
			params.setDI(1);
			teacher->sendSystemMessage(params);

			awardExperience(teacher, "rp_force_training", 1, false);
		}
