...
Luna<LuaSkillManager>::RegType LuaSkillManager::Register[] = {
		...
		{ "getQualifiedForRpSkill", &LuaSkillManager::getQualifiedForRpSkill },
		{ 0, 0 }
};
...
int LuaSkillManager::getQualifiedForRpSkill(lua_State* L) {
	String skillName = lua_tostring(L, -1);
	CreatureObject* creo = (CreatureObject*) lua_touserdata(L, -2);

	lua_pushboolean(L, BorSkill::GetQualifiedForSkill(creo, skillName));
