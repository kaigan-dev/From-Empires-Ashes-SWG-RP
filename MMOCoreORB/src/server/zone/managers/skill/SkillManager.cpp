...
bool SkillManager::surrenderSkill(const String& skillName, CreatureObject* creature, bool notifyClient, bool checkFrs) {
	...
	if (ghost != nullptr) {
		...
		if (skill == "telekinesis" || skill == "control" || skill == "alter" || skill == "inward" || skill == "lightning" || skill == "lightsaber" || skill == "sense") {
			playerManager->awardExperience(target, "rp_frc_skill_cap", 1);
		}
...
int SkillManager::getTrainingSkillRank(CreatureObject* creature, const String& skillPrefix) {
	...
	if(creature->hasSkill(skillPrefix + "_master"))
		return 12;
	else if(creature->hasSkill(skillPrefix + "_rank_10"))
		return 11;
	else if(creature->hasSkill(skillPrefix + "_rank_09"))
		return 10;
	else if(creature->hasSkill(skillPrefix + "_rank_08"))
		return 9;
	else if(creature->hasSkill(skillPrefix + "_rank_07"))
		return 8;
	else if(creature->hasSkill(skillPrefix + "_rank_06"))
		return 7;
	else if(creature->hasSkill(skillPrefix + "_rank_05"))
		return 6;
	else if(creature->hasSkill(skillPrefix + "_rank_04"))
		return 5;
	else if(creature->hasSkill(skillPrefix + "_rank_03"))
		return 4;
	else if(creature->hasSkill(skillPrefix + "_rank_02"))
		return 3;
	else if(creature->hasSkill(skillPrefix + "_rank_01"))
		return 2;
	else if(creature->hasSkill(skillPrefix + "_novice"))
		return 1;
	else return 0;
}
