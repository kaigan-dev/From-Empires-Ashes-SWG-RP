...
bool CreatureImplementation::slotPassenger(CreatureObject* passenger) {
	...
	Locker slocker(seatObject, asCreatureObject());
	seatObject->setCreatureLink(passenger);
	transferObject(seatObject, 4 + seatNumber, true);
	seatObject->setPosition(x, z, y);
	slocker.release();

	//Lambda the player in
	Core::getTaskManager()->executeTask([=]() {
		Locker plocker(passenger);
		passenger->executeObjectControllerAction(STRING_HASHCODE("mount"), seatObject->getObjectID(), "");
		passenger->setPosition(x, z, y);
		plocker.release();
		if (passenger->getParentID() != seatObject->getObjectID()){
			Locker slocker(seatObject);
			seatObject->destroyObjectFromWorld(true);
		}
...
bool CreatureImplementation::slotEscort(CreatureObject* passenger) {
	//get everything necessary for the passenger seat
	int seatNumber = getOpenSeat();
	String seat = "passenger_" + getPassengerSeatName() + "_" + String::valueOf(seatNumber);
	Zone* zone = getZone();
