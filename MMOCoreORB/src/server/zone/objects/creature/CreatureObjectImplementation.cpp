...
void CreatureObjectImplementation::notifyInsert(QuadTreeEntry* obj) {
	...
	if (linkedCreature != nullptr && linkedCreature->getParent() == asCreatureObject()) {
		...
		if (isPassengerSeat())
			linkedCreature->setPosition(getPositionX(), getPositionZ(), getPositionY());
	}
		...
		for (int i = 1; i < 8; ++i) {
			...
			if (seat != nullptr) {
				Core::getTaskManager()->executeTask([=]() {
					...
					seat->notifyInsert(obj);
				}, "passengerNotifyInsert");
			}
		}
	}
	...
	if (hasRidingCreature()) {
		...
		for (int i = 1; i < 8; ++i) {
			...
			if (seat != nullptr) {
				Core::getTaskManager()->executeTask([=]() {
					seat->setPosition(x, z, y);
					seat->notifyDissapear(obj);
				}, "passengerNotifyDissapear");
			}
		}	
	}
	...
	if (hasRidingCreature()) {
		...
		for (int i = 1; i < 8; ++i) {
			...
			if (seat != nullptr) {
				Core::getTaskManager()->executeTask([=]() {
					seat->setPosition(x, z, y);
					seat->notifyPositionUpdate(entry);
				}, "passengerNotifyPositionUpdate");
			}
		}	
	}
...
int CreatureObjectImplementation::getRpForceLevel() const {
	int forceLevel = 0;

	if(hasSkill("rp_force_prog_master")) {
		forceLevel = 6;
	} else if(hasSkill("rp_force_prog_rank_04")) {
		forceLevel = 5;
	} else if(hasSkill("rp_force_prog_rank_03")) {
		forceLevel = 4;
	} else if(hasSkill("rp_force_prog_rank_02")) {
		forceLevel = 3;
	} else if(hasSkill("rp_force_prog_rank_01")) {
		forceLevel = 2;
	} else if(hasSkill("rp_force_prog_novice")) {
		forceLevel = 1;
	}
...
void CreatureObjectImplementation::removeOutOfRangeObjects() {
	...
	if (creature == nullptr || creature->isPassengerSeat())
		return;
	...
	if (parent != nullptr && parent->isPassengerSeat())
		return;
