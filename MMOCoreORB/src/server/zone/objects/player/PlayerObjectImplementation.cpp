...
int PlayerObjectImplementation::addExperience(const String& xpType, int xp, bool notifyClient) {
	...
	//Don't really want to cap RP general experience. 
	if(xpType == "rp_general") {
		xpCap = 5100000;
	}
...
bool PlayerObjectImplementation::addSchematics(Vector<ManagedReference<DraftSchematic* > >& schematics, bool notifyClient) {
	if (schematics.size() == 0) {
		//error("Adding Schematic to Player: Schematics size was 0.");
		return false;
	}
	...
	if (schematicsToSend.size() == 0) {
		//error("Adding Schematic to Player: SchematicToSend size was 0.");
		return false;
	}
...
void PlayerObjectImplementation::doRecovery(int latency) {
	...
	*/
	...
	if (cooldownTimerMap->isPast("roleplayExpEvent") && creature->getStoredInt("exp_banned") != 1) {
		if(getAdminLevel() == 0 && !isAFK()) {
			...
			//Score is set to X + chatScore, and with every tick since a message hasn't been sent, it goes down. 
			int baseScore = (pMan->getBaseImmersiveExp() + chatScore) / ticks;

			creature->setStoredInt("exp_chat_score", 0);

			if(baseScore > 1)
				pMan->awardExperience(creature, "rp_general", baseScore);
			

			//Daily Faction Involvement
			if(creature->getStoredString("faction_current") != "" && baseScore > 1) {
				int currentFood = getFoodFilling();
				int maxFood = getFoodFillingMax();
				if(currentFood < maxFood) {
					int foodAddScore = currentFood + (baseScore / 2) + 1;
					//Prevent overflow
...
void PlayerObjectImplementation::recalculateRoleplayHAM(bool notifyClient) {
	...
	if(player->getHAM(0) > health)
		player->setHAM(0, health, true);
	if(player->getHAM(1) > health)
		player->setHAM(1, health, true);
	if(player->getHAM(2) > health)
		player->setHAM(2, health, true);
	...
	if(player->getHAM(3) > action)
		player->setHAM(3, action, true);
	if(player->getHAM(4) > action)
		player->setHAM(4, action, true);
	if(player->getHAM(5) > action)
		player->setHAM(5, action, true);
	...
	if(player->getHAM(6) > will)
		player->setHAM(6, will, true);
	if(player->getHAM(7) > will)
		player->setHAM(7, will, true);
	if(player->getHAM(8) > will)
		player->setHAM(8, will, true);

	if (getJediState() > 0) {
		setForcePowerMax(force, notifyClient);
		if(getForcePower() > getForcePowerMax()) 
			setForcePower(force, notifyClient);
	} else {
