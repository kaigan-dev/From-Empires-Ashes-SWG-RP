...
void SceneObjectImplementation::updateVehiclePosition(bool sendPackets) {
	...
	if (parent == nullptr || isPassengerSeat() || parent->isPassengerSeat() || (!parent->isVehicleObject() && !parent->isMount()))
		return;
...
bool SceneObjectImplementation::isPassengerSeat() const {
	return templateObject->getFullTemplateString().contains("passenger");
}
