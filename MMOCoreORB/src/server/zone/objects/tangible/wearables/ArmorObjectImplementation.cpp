...
void ArmorObjectImplementation::loadTemplateData(SharedObjectTemplate* templateData) {
	...
	if(!wasModified) {
		...
		if ((armorTemplate->getRating() != nullptr)) {
		...
		if ((armorTemplate->getRpSkillLevel() != nullptr)) {
...
void ArmorObjectImplementation::fillAttributeList(AttributeListMessage* alm, CreatureObject* object) {
	...
	if(getRpSkillLevel() > 0) {
		alm->insertAttribute("skill_level", getRpSkillLevel());
	}
	...
	if (getLightSaber() > 0) {
		StringBuffer txt;
		alm->insertAttribute("rp_special.rp_arm_ls", "Yes.");
	}
